...
Cultivation_caste |>
  ...
  summarise(
    ...
    `Champs par ménage` = round(`Nombre de champs`/`Nombre de ménages`, 1),
    .groups = "drop"
  ) |>
  arrange(desc(`Champs par ménage`)) |>
  gt() |>
  tab_header(
...
A REFAIRE, car les champs sont comptés par saison (donc un même champ est compté deux fois dans l'année) alors que les ménages ne sont comptés qu'une fois dna sl'année, cela doouble le nombre de champs détenu par ménage


```{r}
plot_per_hh <- Cultivation_caste |>
  distinct(VDS_ID, SEASON, PLOT_CODE, CROP) |>
  group_by(VDS_ID, SEASON) |>
  summarise(nb_cultures = n_distinct(CROP), .groups = "drop")
