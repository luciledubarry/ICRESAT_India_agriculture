...
```{r}
Table_labor_plot_all <- readRDS(here::here("Base de données générées", "Table_labor", "Table_labor_plot_all.rds"))

Table_labor_hh_all <- readRDS(here::here("Base de données générées", "Table_labor", "Table_labor_hh_all.rds"))
...
dup_plots_paddy <- Table_labor_plot_all |>
...
```{r}
cols_labor <- c(
  paste0("WORK_HR_", c("HM", "FM", "FF", "HF", "HC", "OB", "RS", "EF", "EM", "FC", "EB", "T")),
  paste0("HACRE_", c("HM", "FM", "FF", "HF", "HC", "OB", "RS", "EF", "EM", "FC", "EB", "T"))
)

test_work_T <- Cultivation_expand_all |>
  select(YEAR, VDS_ID, SUB_PLOT_CODE, SEASON, CROP, CROP_AREA, OPERATION, DT_OPER, all_of(cols_labor)) |>
  select(-WORK_HR_T) |>
  mutate(total_hrs = rowSums(across(contains("WORK_HR")), na.rm = TRUE),
         HACRE_T = round(HACRE_T, 2),
         check_hacre = round(total_hrs/CROP_AREA, 2),
         diff_hacre = check_hacre != HACRE_T) |>
  select(YEAR, VDS_ID, SUB_PLOT_CODE, SEASON, CROP, OPERATION, DT_OPER, HACRE_T, CROP_AREA, total_hrs, check_hacre, diff_hacre)
```
...
Table_labor_plot_all <- convertir_facteur(Table_labor_plot_all, vars_facteurs)
```
...
Table_labor_plot_all$SOIL_TYPE <- relevel(Table_labor_plot_all$SOIL_TYPE, ref = "1")
Table_labor_plot_all$SOIL_DEGR <- relevel(Table_labor_plot_all$SOIL_DEGR, ref = "1")
```
...
```{r}
Table_labor_plot_all <- Table_labor_plot_all |>
  mutate(
...
reg_all_oper <- lm(HACRE_T ~ CASTE_GROUP + CROP_AREA + CROP_AREA_SQ + AREA_HH 
                   ...
                   + STATE, data = Table_labor_plot_all)
