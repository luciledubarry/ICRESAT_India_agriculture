...
vars_facteurs <- c("CASTE_GROUP", "VILLAGE", "STATE", "REGION")
...
```{r vars_dummies}
Cultivation_oper_all <- Cultivation_oper_all |>
  mutate(
...
reg_all_oper <- Cultivation_oper_all |>
  ...
  lm(formula = HACRE_T ~ CASTE_GROUP + CROP_AREA + CROP_AREA_SQ + AREA_HH + AREA_HH_SQ + IRRIGATION + mechanized + mechanized_tilling + YEAR + STATE, data = _)
...
reg_trans <- Cultivation_oper_all |>
  ...
  lm(formula = HACRE_T ~ CASTE_GROUP + CROP_AREA + STATE, data = _)
summary(reg_trans)
```
...
reg_min <- Cultivation_oper_all |>
  ...
  lm(formula = HACRE_T ~ CASTE_GROUP + CROP_AREA + CROP_AREA_SQ + STATE, data = _)
summary(reg_min)
```
...
reg_FF <- Cultivation_oper_all |>
  ...
  lm(formula = HACRE_FF ~ CASTE_GROUP + CROP_AREA + CROP_AREA_SQ + AREA_HH + AREA_HH_SQ + IRRIGATION + mechanized + mechanized_tilling + YEAR + STATE, data = _)
summary(reg_FF)
```
...
reg_HF <- Cultivation_oper_all |>
  filter(SEASON == "KHARIF", CROP == "PADDY") |>
  lm(formula = HACRE_FF ~ CASTE_GROUP + CROP_AREA + CROP_AREA_SQ + AREA_HH + AREA_HH_SQ + IRRIGATION + mechanized + mechanized_tilling + OPERATION + YEAR + STATE, data = _)
summary(reg_HF)
```
...
reg_FM <- Cultivation_oper_all |>
  filter(SEASON == "KHARIF", CROP == "PADDY") |>
  lm(formula = HACRE_FM ~ CASTE_GROUP + CROP_AREA + CROP_AREA_SQ + AREA_HH + AREA_HH_SQ + IRRIGATION + mechanized + mechanized_tilling + YEAR + STATE, data = _)
summary(reg_FM)
```
...
reg_HM <- Cultivation_oper_all |>
  filter(SEASON == "KHARIF", CROP == "PADDY") |>
  lm(formula = HACRE_HM ~ CASTE_GROUP + CROP_AREA + CROP_AREA_SQ + AREA_HH + AREA_HH_SQ + IRRIGATION + mechanized + mechanized_tilling + YEAR + STATE, data = _)
summary(reg_HM)
```
...
reg1_hh <- Cultivation_hh_all |>
  ...
  lm(formula = HACRE_FF ~ CASTE_GROUP + AREA_HH + AREA_HH_SQ + YEAR + VILLAGE, data = _)
summary(reg1_hh)
```
...
reg2_hh <- Cultivation_hh_all |>
  filter(VILLAGE_CROP == "PADDY", SEASON == "KHARIF") |>
  lm(formula = with_paddy ~ CASTE_GROUP + AREA_HH + AREA_HH_SQ + YEAR + VILLAGE, data = _)
summary(reg2_hh)
```
...
reg3_hh <- Cultivation_hh_all |>
  filter(VILLAGE_CROP == "PADDY", SEASON == "KHARIF") |>
  lm(formula = paddy_prct ~ CASTE_GROUP + AREA_HH + AREA_HH_SQ + YEAR + VILLAGE, data = _)
summary(reg3_hh)
```
...
# Ajouter \small
table_tex_wrapped <- paste0("\\small\n", table_tex)
...
reg4_plot <- Cultivation_plot_all |>
  ...
  lm(formula = hand_weeding_plot ~ CASTE_GROUP + CROP_AREA + CROP_AREA_SQ + PLOT_AREA
     + PLOT_AREA_SQ + AREA_HH + AREA_HH_SQ + YEAR + VILLAGE + SOIL_TYPE + SOIL_FERT 
...
reg6_plot_1 <- Cultivation_oper_all |>
  filter(CROP == "PADDY",
         VILLAGE_CROP == "PADDY",
         SEASON == "KHARIF") |>
...
reg6_plot_2 <- Cultivation_oper_all |>
  filter(CROP == "PADDY",
         VILLAGE_CROP == "PADDY",
         SEASON == "KHARIF") |>
...
table_tex <- modelsummary(
  ...
  escape = FALSE,
  add_rows = data.frame(
    Variable = c("Year FE", "Village FE", "Soil characteristics FE"),
...
reg7_oper <- lm(mechanized ~ CASTE_GROUP + CROP_AREA + CROP_AREA_SQ + PLOT_AREA 
                + PLOT_AREA_SQ + AREA_HH + AREA_HH_SQ + OPERATION + YEAR + VILLAGE 
...
table_tex <- modelsummary(
  ...
  escape = FALSE,
  add_rows = data.frame(
    Variable = c("Year / Village / Operation FE", "Soil characteristics FE"),
