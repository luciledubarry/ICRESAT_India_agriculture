...
```{r}
Cultivation_caste <- readRDS(here::here("Base de données générées", "Cultivation_caste", "Cultivation_caste.rds"))
```

Analyse de la base Cultivation_caste, pour l'année 2014
...
n_menages <- Cultivation_caste |>
  distinct(VDS_ID) |>
  summarise(nb = n()) |>
  pull(nb)

n_champs <- Cultivation_caste |>
  distinct(VDS_ID, PLOT_CODE, SEASON) |>
  summarise(nb = n()) |>
  pull(nb)

n_crops <- Cultivation_caste |>
  distinct(CROP) |>
  summarise(nb = n()) |>
  pull(nb)
...
plot_cultures <- Cultivation_caste |>
...
```{r}
n_menages_region <- Cultivation_caste |>
  distinct(VDS_ID, REGION) |>
  count(REGION, name = "nb_menages")

n_plots_region <- Cultivation_caste |>
...
J'applique différents filtres sur Cultivation_caste avant le calcul du ratio Hr/acre par champ. Filtres appliqués : 
...
top_25_crops <- Cultivation_caste |>
...
Cultivation_top_25 <- Cultivation_caste |>
...
```{r}
crops_village <- Cultivation_caste |>
  filter(CROP != "SEASONAL FALLOW") |>
...
```{r}
main_crop_village <- Cultivation_caste |>
  filter(CROP != "SEASONAL FALLOW") |>
...
top_10_crops <- Cultivation_caste |>
  filter(CROP != "SEASONAL FALLOW") |>
  distinct(VDS_ID, PLOT_CODE, SEASON, CROP, AREA_CROP) |> 
  count(CROP, name = "nb_occurrences") |>
  arrange(desc(nb_occurrences)) |>
...
hr_per_gender <- Cultivation_caste |>
...
```{r}
hired_work_all <- Cultivation_caste |>
...
```{r}
oper_per_month <- Cultivation_caste |>
  filter(
    EMPLOYMENT == "Family",
    GENDER == "Male"
  ) |>
...
```{r}
oper_per_gender <- Cultivation_caste |>
...
test_ratio_oper <- Cultivation_caste |>
...
oper_gender_prct <- Cultivation_caste |>
  filter(GENDER %in% c("Female", "Male")) |>
...
A faire: Calculer la variable IRRIGATION dans Cultivation_caste puis l'ajouter à Cultivation_top_25
...
J'utilise Cultivation_top_25, car les valeurs de HYV sont renommées seulement dans cette table (à faire dans Cultivation_caste)
