---
title: "Fichier 3 : Analyse de la base Cultivation 2014 (partie 2)"
format: html
editor: source
---
...
```{r, message=FALSE, warning=FALSE}
here::i_am("ICRESAT_database_analysis/CultData3_analysis_2.qmd")
library(dplyr)
library(tidyr)
library(ggplot2)
library(scales)
...
Cette base inclut les opérations non complétées (c’est-à-dire celles avec un temps de travail égal à 0), pour lesquelles on a ajouté des lignes. 
...
J'applique différents filtres sur Cultivation_expand avant le calcul du ratio Hr/acre par champ. Filtres appliqués : 
...
- Enlever les lignes en doublons (Je sais pas pourquoi j'en ai, regarder dans le fichier 2 CultData_cleaning)
...
On regarde la variété de cultures faites par village, pour vérifier s'il y a des villages "blés" et des villages "riz". 

Pour cela, je compte le nombre de cultures différentes dans chaque village. 
...
```{r}
ggsave("figures/2nd_crop_village.png", plot = plot_first_crop, width = 8, height = 6, dpi = 300)
```
...
```{r}
ggsave("figures/2nd_crop_village.png", plot = plot_second_crop, width = 8, height = 6, dpi = 300)
```
...
# Étape 2: Marquer si un ménage cultive la crop de rang 1
...
hr_per_acre |>
  ...
  labs(
    x = "Hr/acre par champ",
    y = "Nombre de champs",
    title = "Paddy: Distribution de Hr/acre (seulement travail humain)"
  )
```
...
L'hypothèse semble vérifiée (à regarde plus en détails) : les femmes travaillent plus sur les opérations non méchanisées. 


Questions suivantes : 

- Pour les champs qui ont cultivé du riz en première saison (Kharif), quels sont leurs choix de culture en Rabi (culture ou jachère) ? 

en filtrant pour les champs irrigués / non-irrigués ? 

- regarder quels sont les choix en deuxième saison par caste : proportion de champs cutlivant les 4 principaux crops en Rabi en fonction de la caste

- par ménage, combien de crops cultivés par saison (nombre champs avec des crops distincts). calculer notamment dans les villages riz en Kharif, combien de ménages ont une autre culture (en plus du riz) et quelle est la proportion de terre (area_crop) dans un ménage qui ne soit pas du riz

- Graphique: Hr/acre par acre par caste -> les faire en plus par ménage (plutôt que par champ) --> calucler la supericie totale par méange et le temps de travail total  --> pour toujours filtrer pour le riz, définir des ménages riz qui ont un pourcentage de terre alloué au riz suffisamment haut
...
paddy_kharif <- Cultivation_expand |>
...
crop_choice_rabi <- Cultivation_expand |>
  left_join(paddy_kharif, by = c("VDS_ID", "PLOT_CODE")) |>
