...
FAUX: car si on somme tous les champs d'un ménage par saison, on voit que pour la très grande majorité (tous excepté 2 champs) les superficies sont égales entre els deux saisons. Donc, les champs ayant indiqué FALSE dans la partie précédente sont juste de la consolidation de champs parmi le même ménage. 
...
cat("Sur un total de", nrow(cultivation_wider_season |> filter(diff_area == FALSE)), "champs présents dans les deux saisons soit", round(100 * nrow(cultivation_wider_season |> filter(diff_area)) / nrow(cultivation_wider_season |> filter(diff_area == FALSE)), 1), "%\n")
```
```{r}
test_area_hh <- cultivation_wider_season |>
  group_by(VDS_ID) |>
  summarise(
    area_kharif = sum(KHARIF),
    area_rabi = sum(RABI), .groups = "drop"
  ) |>
  ungroup() |>
