...
### Identifier les champs de façon unique
...
dup_rows_champs <- Cultivation_expand |>
  distinct(VDS_ID, SUB_PLOT_CODE, CROP, SEASON) |>
  group_by(VDS_ID, SUB_PLOT_CODE, SEASON, CROP) |>
  filter(n() > 1) |>
  arrange(VDS_ID, SUB_PLOT_CODE, SEASON, CROP) |>
  mutate(row_in_group = row_number()) |> 
  ungroup()
```

Pour ces 3 champs, je modifie PLOT_CODE pour différencier les doublons.

```{r}
dup_rows_champs <- dup_rows_champs |>
  mutate(PLOT_CODE_NEW = ifelse(row_in_group == 2, "Z", PLOT_CODE)) |>
  select(VDS_ID, CROP, SEASON, PLOT_CODE, PLOT_CODE_NEW)

Cultivation_expand <- Cultivation_expand |>
  left_join(dup_rows_champs, by = c("VDS_ID", "CROP", "SEASON", "PLOT_CODE")) |>
...
Cultivation_expand <- full_season |>
