...
Avant de recatégoriser les opérations en une dizaine d'opérations, je créé les variables Fertilizer et Herbicide qui indiquent si l'opération a nécessité de l'engrais ou de l'herbicide respectivement; 
...
A MODIFIER : Ce code est sans intérêt car les opérations sont déjà harmonisées (donc ne précisent pas si de l'engrais a été utilisé) !

```{r}
# Variable HERBICIDE
# L'opération implique de l'herbicide/weedicide si l'une des deux conditions est remplie. 
Cultivation_cult_distinctes <- Cultivation_cult_distinctes |>
  mutate(
...
J'identifie les méanges ayant au moins un champ de riz par saison (peu importe la superficie). 
