...
Cultivation_expand |>
  ...
  summarise(
    ...
    `Nombre de champs` = n_distinct(paste(VDS_ID, PLOT_CODE, sep = "_"))
  ) |>
  gt() |>
  tab_header(
...
### Présence des champs selon la saison
...
bind_rows(
  uniques_kharif |>
    summarise(Saison = "KHARIF seulement", `Nombre de ménages` = n()),
  uniques_rabi |>
    summarise(Saison = "RABI seulement", `Nombre de ménages` = n())
) |>
  gt() |>
  tab_header(title = "Ménages par saison de culture") |>
  cols_label(
    Saison = "Saison",
    `Nombre de ménages` = "Ménages concernés"
  )
```
