...
dup_rows_champs <- Cultivation_expand |>
  ...
  group_by(VDS_ID, PLOT_CODE, SEASON) |>
  filter(n() > 1) |>
  arrange(VDS_ID, PLOT_CODE, SEASON, CROP) |>
...
Pour retomber sur le total de 3 956 champs distincts, calculer Kharif seulement + Rabi seulement + Kharif et Rabi x 2
...
Pour vérifier les différences de superficie des champs entre les saisons (au cas où les champs sont consolidés), pivoter en large la base pour les saisons (pour le superficie et éventuellement la culture). Objectif : vérifier si la superficie d'un mêm champ diffère entre deux saisons et compter le nombre de NA = champs en jachère
