...
Faire le graph du travail total (toutes les catégories) selon la superficie par caste
-> sans mécanisation
-> distinguer pour les principales opérations
-> bullocks ? (regarder utilisation + importance des bullocks selon la caste + dans quelles opérations ils sont utilisés)
-> ajouter land preparation
...
mechanized_tilling <- Cultivation_expand |>
  ...
  reframe(mechanized_tilling = any(MACHINERY == "Mécanisé", na.rm = TRUE)) 
...
```{r}
# Ajouter les heures de travail totales (toutes opérations confondues)
hacre_all_labor <- hacre_all_labor |>
