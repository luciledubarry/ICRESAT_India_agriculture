...
```{r, message=FALSE, warning=FALSE}
here::i_am("ICRESAT_database_analysis/scripts_2014/Regression_labor_2014.qmd")
library(dplyr)
library(tidyr)
library(readxl)
library(ggplot2)
...
```{r}
test_dups <- Cultivation_expand |>
  select(-c(NAME_MAT, TYPE_MAT)) |>
  distinct()
  
any(duplicated(test_dups[, c("VDS_ID", "PLOT_CODE", "SUB_PLOT_CODE", "AREA_CROP", "SEASON", "CROP", "OPERATION", "LAB_TYPE", "WORK_HR", "DT_OPER", "MONTH", "SUR_MON_YR")]))

# Vérifier les doublons avant le calcul de WORK_HR
dup_Cultivation_expand <- test_dups |>
  distinct() |>
...
ATTENTION : Il y a des doublons dans Cultivation_expand à cause de NAME_MAT, TYPE_MAT et peut_etre VAR_TYPE 


```{r}
Cultivation_test <- Cultivation_expand |>
  mutate(LAB_TYPE = ifelse(is.na(LAB_TYPE), "Missing", LAB_TYPE)) |>
  # Modifier LAB_TYPE permet de conserver dans le group_by suivant les lignes LAB_TYPE = n/a donc de conserver les opérations pour lesquelles WORK_HR = 0
  filter(LAB_TYPE != "6") |>
  group_by(VDS_ID, PLOT_CODE, SUB_PLOT_CODE, SEASON, CROP, LAB_TYPE, OPERATION, .keep = "all") |>
