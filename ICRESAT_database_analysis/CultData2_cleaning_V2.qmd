---
title: "Fichier 2 (V3) : Création des tables"
format: html
editor: source
params:
...
table_mat <- table_mat |>
  filter(TYPE_MAT %in% c("WEEDICIDE", "PESTICIDE", "FERTILIZER"),
         ...
         UNIT_MAT != "HR") |>
   mutate(
    QTY_MAT = if_else(UNIT_MAT == "GM", QTY_MAT / 1000, QTY_MAT),
    UNIT_MAT = if_else(UNIT_MAT == "GM", "KG", UNIT_MAT),
    QTY_MAT = if_else(UNIT_MAT == "ML", QTY_MAT / 1000, QTY_MAT),
    UNIT_MAT = if_else(UNIT_MAT == "ML", "LT", UNIT_MAT),
    # QT = quintal (1 quintal = 100 KG) ?
...
group_vars <- c("VDS_ID", "CASTE_GROUP", "SEASON", "CROP", "PLOT_CODE", 
                ...
                "FERTILIZER_KG", "FERTILIZER_LT", "PESTICIDE_KG", "PESTICIDE_LT", 
                "WEEDICIDE_KG", "WEEDICIDE_LT")

Cultivation_oper <- sum_cols_labor(Cultivation_oper, group_vars, cols_labor)
```
...
Cultivation_plot <- sum_cols_labor(Cultivation_plot, c(group_vars, "mechanized"), cols_sum_plot)
```
...
group_vars <- c("VDS_ID", "CASTE_GROUP", "SEASON", "AREA_HH", "REGION", "STATE", "VILLAGE", 
                "VILLAGE_CROP", "with_paddy", "paddy_prct")

Cultivation_hh <- sum_cols_labor(Cultivation_hh, group_vars, cols_sum_plot)
```
