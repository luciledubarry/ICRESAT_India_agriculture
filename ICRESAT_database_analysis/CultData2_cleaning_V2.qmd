...
use_typemat <- function(df, type_mat, nom_var) {
  ...
  df <- left_join(df, info_typemat, by = c("VDS_ID", "PLOT_CODE", "SUB_PLOT_CODE", "CROP_AREA", "CROP", "SEASON", "OPERATION"))
  
  # Remplacer les n/a par FALSE
  df[[nom_var]] <- replace_na(df[[nom_var]], FALSE)

  df
}

Cultivation_long <- Cultivation_long |>
  use_typemat("MACHINERY", "use_machinery") |>
  use_typemat("FERTILIZER", "use_fertilizer") |>
  use_typemat("PESTICIDE/FUNGICIDE", "use_pesticide") |>
  use_typemat("WEEDICIDE", "use_weedicide")
```
...
# Identifier les champs mécanisant land preparation 
  ...
  summarise(mechanized_tilling = any(use_machinery), .groups = "drop")
...
vars_existantes <- intersect(cols_work_hr, colnames(Cultivation_long))

Cultivation_long[vars_existantes] <- lapply(Cultivation_long[vars_existantes], function(x) ifelse(is.na(x), 0, x))

Cultivation_long <- Cultivation_long |>
  mutate(WORK_HR_T = rowSums(across(all_of(vars_existantes))),
...
Je créé la variable hand weeding pour identifier ayant fait du weeding sans utiliser de machine ni produit, et la variable chemical weeding pour ceux ayant utilisé du weedicide (impliquant généralement un sprayer).

```{r}
Cultivation_long <- Cultivation_long |>
  mutate(chemical_weeding = OPERATION == "WEEDING" & WORK_HR_T > 0 & use_weedicide,
         hand_weeding = OPERATION == "WEEDING" &
                        WORK_HR_T > 0 &
                       (is.na(use_weedicide) | !use_weedicide) &
                       (is.na(use_machinery) | !use_machinery)
  )
```
...
```{r}
n_avant <- nrow(Cultivation_long)
t_work_hr <- Cultivation_long |>
  select(-DT_OPER) |>
  distinct()

n_avant - nrow(t_work_hr)
```
...
base_to_join <- Cultivation_long |>
  select(
    ...
    MONTH, use_machinery, use_fertilizer, use_pesticide, use_weedicide, cols_expand
  )
...
Cultivation_expand <- Cultivation_expand |>
  mutate(
    ...
    across(
      c("use_fertilizer", "use_machinery", "use_pesticide", "use_weedicide"),
      ~ replace_na(.x, FALSE)
    )
  )

Cultivation_expand <- convertir_num(Cultivation_expand, vars_existantes)
```
...
group_vars <- c("VDS_ID", "CASTE_GROUP", "SEASON", "CROP", "PLOT_CODE", 
                ...
                "IRRIGATION", "OPERATION", "mechanized_tilling", "use_machinery",
                "use_fertilizer", "use_pesticide", "use_weedicide", "REGION",
...
Table_labor_plot <- sum_cols_labor(Table_labor_plot, group_vars, vars_existantes)
```
...
Table_labor_hh <- sum_cols_labor(Table_labor_hh, group_vars, vars_existantes)
```


## Chemicals

La table suivante indique au niveau du champ (toutes opérations confondues), quels produits ont été utilisés dnas la saison et en quelle quantité (avec l'unité).   

```{r table_chemicals}
# Sélectionner les variables
Table_chemicals <- Cultivation_expand |>
  select(VDS_ID, SEASON, CROP, PLOT_CODE, SUB_PLOT_CODE, CASTE_GROUP, AREA_HH, PLOT_AREA, CROP_AREA, IRRI_AREA, IRRIGATION, use_fertilizer, use_pesticide, use_weedicide, contains(c("TYPE_MAT", "UNIT_MAT", "QTY_MAT")), with_paddy, paddy_prct, SOIL_TYPE, SOIL_FERT, SOIL_DEGR, SLOPE, SOIL_DEPTH, VILLAGE, STATE, REGION)

# Pivoter la table en long : variables _MAT
pattern_vars <- c("TYPE_MAT_", "NAME_MAT_", "UNIT_MAT_", "QTY_MAT_")

cols_mat <- names(Table_chemicals)[
  sapply(pattern_vars, function(p) grepl(p, names(Table_chemicals))) |>
    apply(1, any)
]

Table_chemicals <- Table_chemicals |>
  pivot_longer(
...
# Conserver toutes les lignes avec matériel et une seule ligne par champ s’il n’y a aucun matériel déclaré
  group_by(VDS_ID, CASTE_GROUP, PLOT_CODE, SUB_PLOT_CODE, SEASON, CROP, CROP_AREA) |>
  filter(!is.na(TYPE_MAT) | (all(is.na(TYPE_MAT)) & mat_index == "1")) |>
  ungroup() |>
  select(-mat_index)
```

```{r sum_chemicals}
Table_chemicals <- convertir_num(Table_chemicals, "QTY_MAT")

# Additionner les quantités par champ pour le même type de produit
Table_chemicals <- Table_chemicals |>
  mutate(TYPE_MAT = replace_na(TYPE_MAT, "NONE"),
         QTY_MAT = if_else(TYPE_MAT == "NONE", 0, QTY_MAT)) |>
  group_by(VDS_ID, SEASON, CROP, PLOT_CODE, SUB_PLOT_CODE, CASTE_GROUP, AREA_HH, PLOT_AREA, CROP_AREA, IRRI_AREA, IRRIGATION, TYPE_MAT, UNIT_MAT, with_paddy, paddy_prct, SOIL_TYPE, SOIL_FERT, SOIL_DEGR, SLOPE, SOIL_DEPTH, VILLAGE, STATE, REGION) |>
...
```{r save_table_chemicals}
# Définir le chemin du dossier
folder_path <- here("Base de données générées", "Table_chemicals")
...
# Enregistrer Table_chemicals
name_table <- paste0("Table_chemicals_", annee)
write.csv(
  Table_chemicals,
  file = file.path(folder_path, paste0(name_table, ".csv")),
  row.names = FALSE
