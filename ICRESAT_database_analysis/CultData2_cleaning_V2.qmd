...
*Ménages faisant du hand weeding par saison*
...
plots_by_household <- Cultivation_long |>
  distinct(
    ...
    RENT_FOR, OW_STAT, OP_MAIN_PROD_QTY, OP_MAIN_PROD_UNIT, hand_weeding
           )
...
group_vars <- c("SUR_YR", "VDS_ID", "CASTE_GROUP", "SEASON", "CROP", "PLOT_AREA",
                "CROP_AREA", "IRRI_AREA", "IRRIGATION", "PLOT_CODE", "SUB_PLOT_CODE",
                "OPERATION", "mechanized_tilling", "REGION", "STATE", "VILLAGE",
                "VILLAGE_CROP", "with_paddy", "paddy_prct", "VAR_TYPE", "VAR_NAME",
                "OP_MAIN_PROD_QTY", "OP_MAIN_PROD_UNIT", "SOIL_TYPE", "SOIL_DEPTH",
                "SOIL_FERT", "SOIL_DEGR", "SLOPE", "hand weeding")

Cultivation_labor <- sum_cols_labor(Cultivation_labor, group_vars, cols_labor)
```


Je crée la variable mechanized, qui indique pour chaque opération si une machine a été utilisé (TR, SP, DS, SM, PT, ET, CH, TH, SD, MK, DP, AUTO ou CYCLE WEEDER). 

```{r}
mat_cols <- Cultivation_labor |> select(contains("TYPE_MAT"))

Cultivation_labor <- Cultivation_labor |>
  mutate(mechanized2 = rowSums(mat_cols == "MACHINERY", na.rm = TRUE) > 0) |>

# Conserver seulement les variables sur le travail
  select(-contains(c("MAT", "WAGE")))
```
...
Cultivation_mat <- Cultivation_expand |>
