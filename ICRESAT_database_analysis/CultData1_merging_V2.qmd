...
CultInput$CROP_AREA <- as.numeric(as.character(CultInput$CROP_AREA))
CultInput$CROP_AREA <- round(CultInput$CROP_AREA, 2)
...
PlotList$IRRI_AREA <- as.numeric(as.character(PlotList$IRRI_AREA))
PlotList$IRRI_AREA <- round(PlotList$IRRI_AREA, 2)

PlotList$CROP_AREA <- as.numeric(as.character(PlotList$CROP_AREA))
PlotList$CROP_AREA <- round(PlotList$CROP_AREA, 2)

PlotList$PLOT_AREA <- as.numeric(as.character(PlotList$PLOT_AREA))
PlotList$PLOT_AREA <- round(PlotList$PLOT_AREA, 2)
```
...
```{r}
Cultivation <- CultInput_merged |>
  select(-PLOT_NAME) |>

# D'abord joindre avec CultOutput
  left_join(
    CultOutput |> select(-c(PLOT_NAME, IRRI_AREA, STATE, SUR_YR, CROP_AREA)),
    by = c("VDS_ID", "SUB_PLOT_CODE", "SEASON")
  ) |>
...
# Remplacer les NA avec les infos de PlotList
  ...
  select(-CROP_1_PL, -CROP_2_PL)
```

```{r}
# Changer l'ordre des colonnes de Cultivation
...
*Correspondance entre Cultivation et Household_info*

Les identifiants des ménages ne correspondent pas entre les deux tables, parce que dans la table Household_info, VDS_ID est majoritairement indiqué pour l'année 2013 plutôt que 2014. 
...
cat(" - Ménages totaux dans Cultivation :", n_hh, "\n")
...
## Enregistrer Cultivation

```{r}
# Définir le chemin du dossier
folder_path <- here("Base de données générées", "Cultivation")
...
write.csv(
  Cultivation,
  file = file.path(folder_path, "Cultivation.csv"),
  row.names = FALSE
)
...
saveRDS(
  Cultivation,
  file = file.path(folder_path, "Cultivation.rds"),
)
```
