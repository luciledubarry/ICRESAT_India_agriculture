...
ATTENTION : Je pense que l'erreur est liée à des lignes en double (tous sauf WORK_HR, HACRE, WAGE)

+ faire attention à ne pas créer des doublons à cause de SUB_OPERATION 

```{r}
dup_CultInput_LAB <- CultInput_LAB |>
  group_by(across(-c(WAGE, WORK_HR, HACRE, SUB_OPERATION))) |>
  filter(n() > 1) |>
  mutate(row_id = row_number()) |> 
  ungroup()
```
