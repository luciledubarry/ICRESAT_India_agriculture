...
*1. CultInput_LAB : Human and bullock labor*
...
*2. CultInput_MAT : Material inputs and machinery use*
...
dup_CultInput_MAT <- CultInput_MAT |>
  group_by(VDS_ID, PLOT_CODE, SUB_PLOT_CODE, CROP_AREA, IRRI_AREA, SEASON, OPERATION, DT_OPER, SUR_MON_YR) |>
  filter(n() > 1) |>
  mutate(row_id = row_number()) |> 
  ungroup()
```
...
Je supprime les lignes pour lesquelles aucune information sur la mécanisation n'est indiquée (soit toutes les variables MAT sont n/a). 

```{r}
CultInput_MAT <- CultInput_MAT |>
  filter(!if_all(ends_with("MAT"), is.na))
```
...
Pas de ligne identique dans PlotList
