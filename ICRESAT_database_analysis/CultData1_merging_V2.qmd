...
any(duplicated(Landholding_2014[, c("VDS_ID", "PLOT_CODE")]))
```
...
```{r}
Cultivation_2014 <- CultInput_merged |>
  select(-PLOT_NAME) |>

# D'abord joindre avec CultOutput
  left_join(
    CultOutput_2014 |> select(-c(PLOT_NAME, IRRI_AREA, STATE, SUR_YR, CROP_AREA)),
    by = c("VDS_ID", "SUB_PLOT_CODE", "SEASON")
  ) |>
  
# Puis joindre avec Landholding 
  left_join(Landholding_2014 |> select(-STATE), by = c("VDS_ID", "PLOT_CODE")) |>
  select(-PLOT_CODE) |>
