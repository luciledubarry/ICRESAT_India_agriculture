...
dup_plots_paddy <- Cultivation_test |>
...
Cultivation_dummy <- cbind(Cultivation_paddy, dummies_state)

Supprimer la première dummy : dummies_state <- dummies_state[, -1] ? 

Pour Village (21 valeurs)
dummies_village <- model.matrix(~ VILLAGE - 1, data = Cultivation_paddy)
Cultivation_dummy <- cbind(Cultivation_dummy, dummies_village)
...
```{r}
reg <- lm(HACRE_OPER ~ CASTE_GROUP + AREA_CROP + AREA_CROP_SQ + AREA_HH + AREA_HH_SQ + IRRIGATION + MACHINERY + mechanized_tilling + VAR_TYPE + STATE, data = Cultivation_paddy)
summary(reg)
```
