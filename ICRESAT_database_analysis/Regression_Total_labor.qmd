...
*Vérifier la variation dans hand weeding* 

Je vérifie s'il y a des ménages qui ne font pas de hand weeding pendant toute la saison. 

```{r}
na_hand_weeding <- Cultivation_expand |>
  filter(OPERATION == "HAND WEEDING") |>
  group_by(VDS_ID, PLOT_CODE, SUB_PLOT_CODE, AREA_CROP, CROP, SEASON) |>
  reframe(na_weeding = any(WORK_HR == 0)) |>
  ungroup()

# Ajouter la variable à Cultivation_expand
Cultivation_test <- Cultivation_expand |>
  left_join(na_hand_weeding, by = c("VDS_ID", "PLOT_CODE", "SUB_PLOT_CODE", "AREA_CROP", "CROP", "SEASON")) |>
  filter(VILLAGE_CROP == "PADDY", CROP == "PADDY", SEASON == "KHARIF")

View(Cultivation_test |> filter(OPERATION == "HAND WEEDING", na_weeding == TRUE) |> distinct(VDS_ID, PLOT_CODE, SUB_PLOT_CODE, AREA_CROP, WORK_HR))
```
...
ATTENTION : Cette méthode pour sommer les heures de travail par opération et lab_type enlève (pour une certaine raison) les champs pour lesquels Work_hr = 0, notammen tpour hand weeding. 

```{r}
Cultivation_expand <- Cultivation_expand |>
  filter(LAB_TYPE != "6") |>
  group_by(across(-"WORK_HR")) |>
  summarise(WORK_HR = sum(WORK_HR), .groups = "drop")
```

```{r}
hand_weeding <- Cultivation_expand |>
  filter(OPERATION == "HAND WEEDING") |>
  select(WORK_HR, VDS_ID, PLOT_CODE, SUB_PLOT_CODE, CROP, SEASON, OPERATION) |>
  mutate(hand_weeding = if_else(WORK_HR == 0, 0, 1))

# Ajouter la variable à Cultivation_expand
Cultivation_expand_test <- Cultivation_expand |>
  left_join(hand_weeding, by = c("VDS_ID", "PLOT_CODE", "SUB_PLOT_CODE", "CROP", "SEASON", "WORK_HR", "OPERATION")) |>
  filter(VILLAGE_CROP == "PADDY", CROP == "PADDY", SEASON == "KHARIF")

View(Cultivation_expand_test |> filter(OPERATION == "HAND WEEDING", hand_weeding == 0) |> distinct(VDS_ID, PLOT_CODE, SUB_PLOT_CODE, AREA_CROP, WORK_HR))
```
...
na_hand_weeding <- Cultivation_expand |>
  filter(OPERATION == "HAND WEEDING") |>
  group_by(VDS_ID, PLOT_CODE, SUB_PLOT_CODE, AREA_CROP, CROP, SEASON) |>
  reframe(na_hand_weeding = any(WORK_HR == 0)) |>
  mutate(na_hand_weeding = replace_na(na_hand_weeding, TRUE))

# Ajouter la variable à Cultivation_expand
Cultivation_expand <- Cultivation_expand |>
  left_join(na_hand_weeding, by = c("VDS_ID", "PLOT_CODE", "SUB_PLOT_CODE", "AREA_CROP", "CROP", "SEASON"))

nrow(Cultivation_expand |> filter(OPERATION == "HAND WEEDING", na_hand_weeding == TRUE) |> distinct(VDS_ID, PLOT_CODE, SUB_PLOT_CODE, AREA_CROP, WORK_HR))
```
