...
Il y a des champs en double à cause de MULTI_CROPING, OP_MAIN_PROD_QTY et OP_MAIN_PROD_UNIT. Une fois ces variables enlevées, il faut distinct la table.  

```{r}
# Supprimer les doublons causés par OP
Cultivation_expand <- Cultivation_expand |>
  group_by(across(-c(OP_MAIN_PROD_QTY, OP_MAIN_PROD_UNIT))) |>
  summarise(
...
# Sélectionner les variables d'intérêt
  select(VDS_ID, CASTE_GROUP, PLOT_CODE, AREA_CROP, OPERATION, LAB_TYPE, WORK_HR, IRRIGATION, NAME_MAT, MACHINERY, VAR_TYPE, REGION, STATE, VILLAGE) |>
...
```{r}
household_plots <- Cultivation_paddy |>
  distinct(VDS_ID, PLOT_CODE, AREA_CROP, .keep_all = TRUE) |>
  group_by(VDS_ID, CASTE_GROUP, VILLAGE) |>
  summarise(AREA_HH = sum(AREA_CROP), .groups = "drop")

Cultivation_paddy <- Cultivation_paddy |>
  left_join(household_plots, by = c("VDS_ID", "CASTE_GROUP", "VILLAGE"))
```
...
reg2 <- lm(HACRE_OPER ~ CASTE_GROUP, data = Cultivation_paddy)
summary(reg2)
```
...
Attention : Pour cette régression, mechanized_tilling et MACHINERY devraient être identiques. La régression devrait sauter, à cause de la colinéarité --> Comme ce n'est pas le cas, vérifier construction de mechanized_tilling et MACHINERY
