...
```{r}
# Enlever le champ dont AREA_CROP = 0, ce qui fausse le calcul de HR_PER_ACRE
Cultivation_expand <- Cultivation_expand |>
  filter(AREA_CROP != 0)
```

Je crée une base pour calculer le ratio Hr/acre par champ (variable HR_PER_ACRE) pour m'assurer que de n'avoir qu'une seule ligne par champ. 

```{r}
# Créer une base pour calculer le ratio Hr/acre par champ (variable HR_PER_ACRE)
hr_per_acre <- Cultivation_expand |>
...
Cultivation_expand <- Cultivation_expand |>
  group_by(VDS_ID, PLOT_CODE, SEASON, CROP, AREA_CROP) |> # grouper par champ distinct
  mutate(
    HR_PER_ACRE = sum(WORK_HR, na.rm = TRUE) / AREA_CROP
  ) |>
  ungroup()
...
### Calcul de la moyenne de Hr/acre pour les principales cultures
...
mean_hacre_crop <- Cultivation_top_25 |>
  group_by(CROP) |>
  distinct(VDS_ID, PLOT_CODE, SEASON, CROP, AREA_CROP, .keep_all = TRUE) |> 
  summarise(SUM_HACRE = sum(HR_PER_ACRE, na.rm = TRUE),
    MEAN_HACRE = mean(HR_PER_ACRE, na.rm = TRUE),
    n_plots = n()
  )
...
```{r Moyenne Hr/acre par culture}
# Moyenne Hr/acre pour toutes les cultures saisonnières (seulement travail humain)
mean_hacre_crop <- Cultivation_top_25 |>
...
### Graph- Hr/acre sans multi-cropping
...
Je ne prends en compte pour le graphique suivant que les champs avec une seule culture (MULTI_CROPING = FALSE) et seulement le travail humain. 

```{r}
# Refaire la moyenne de Hr/acre en enlevant les champs qui ont plusieurs cultures
mean_hacre_crop <- Cultivation_top_25 |>
...
mean_hacre_crop |>
  ...
  coord_flip() +
  labs(
    title = "Top 25: Moyenne de Hr/acre (sans multi-cropping)",
    x = "",
    y = ""
  ) +
  scale_y_continuous(labels = comma) +
...
Cultivation_top_25 |>
  ...
  labs(
    x = "",
    y = "",
    title = "Top 10: Distribution de Hr/acre (seulement travail humain)"
  )
```
...
Normalement, en additonnan tous les Hr/acre moyens par opération, on retombe sur la moyenne de Hr/acre calculée au début. 

#### Ratio- Hr/acre par opération

```{r}
# Calculer le ratio Hr/acre par opération et par champ (variable HR_PER_OPER)
Cultivation_top_25 <- Cultivation_top_25 |>
  group_by(VDS_ID, PLOT_CODE, SEASON, CROP, AREA_CROP, OPERATION) |> 
...
#### Calcul de la moyenne de Hr/acre par opération 

```{r Moyenne Hr/acre par opération}
# Calculer la moyenne de Hr/acre (variable MEAN_HACRE) par opération et par culture, en considérant seulement le travail humain et sans le multi-cropping
mean_hacre_oper <- Cultivation_top_25 |>
  filter(!MULTI_CROPING) |>
  filter(is.na(HUMAN_LABOR) | HUMAN_LABOR == TRUE) |>
  distinct(VDS_ID, PLOT_CODE, SEASON, CROP, AREA_CROP, OPERATION, .keep_all = TRUE) |> 
  group_by(OPERATION, CROP) |>
  summarise(SUM_HACRE = sum(HR_PER_OPER, na.rm = TRUE),
...
```{r test moyenne Hr/acre}
test_mean <- Cultivation_top_25 |>
  filter(!MULTI_CROPING) |>
  filter(is.na(HUMAN_LABOR) | HUMAN_LABOR == TRUE) |>
  filter(CROP == "PADDY" & OPERATION == "LAND PREPARATION") |>
  distinct(VDS_ID, PLOT_CODE, SEASON, CROP, AREA_CROP, OPERATION, .keep_all = TRUE) |> 
  group_by(CROP, OPERATION) |>
  summarise(SUM_HACRE = sum(HR_PER_OPER, na.rm = TRUE),
    MEAN_HACRE = mean(HR_PER_OPER, na.rm = TRUE),
...
#### Graph- Hr/acre moyen par opération

Je refais les graphs avec le Hr/acre moyen par opération plutot que le total d'heures, pour harmoniser les résultats par rapport à la taille des parcelles. 

Ce graphique représente la moyenne de Hr/acre par opération pour le paddy. 
...
Les graphiques suivants comparent le Hr/acre moyen entre les principales cultures pour une opération spécifique. 
...
distinct_plots <- Cultivation_top_25 |> distinct(VDS_ID, PLOT_CODE, CROP, SEASON, AREA_CROP, .keep_all = TRUE)
