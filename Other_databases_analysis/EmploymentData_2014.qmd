...
# Par type de travail employé, enlever les doublons
  distinct(SUR_MON_YR, VDS_ID, PRE_MEM_ID, OLD_MEM_ID, REL, GENDER, AGE, WORK_TYPE, .keep_all = TRUE)
```
...
dup_hired_work <- hrs_worked |>
  filter(!is.na(WORK_TYPE)) |>
  group_by(across(-c(WORK_TYPE, WORK_DAYS, AVG_WORK_HRS, HIRED_WORK_T))) |>
  filter(n() > 1) |>
  ungroup()
```
