...
Il faut vérifier que DT_INT (JJ/MM/AAAA) correspond bien au mois suivant de SUR_MON_YR (MM/AA) tel que DT_INT = SUR_MONT_YR + 1.
...
Employment_2014 <- Employment_2014 |>
  ...
  mutate(
    DT_INT_parsed = parse_date_time(DT_INT, orders = c("dmy", "ymd", "dmY", "d m Y")), # conversion des différents formats
    SUR_MON_YR_parsed = as.Date(paste0("01/", SUR_MON_YR), format = "%d/%m/%y")
  )  # conversion du format MM/YY
```


```{r}
# Nombre de lignes pour lesquelles DT_INT et SUR_MON_YR ne correspondent pas
test_date_int <- test_date_int |>
  mutate(
    SUR_MON_PLUS1 = SUR_MON_DATE %m+% months(1),  # ajouter 1 mois
    same_month = month(DT_INT_DATE) == month(SUR_MON_PLUS1) &
                 year(DT_INT_DATE) == year(SUR_MON_PLUS1)
  )
```
...
hrs_worked <- Employment_2014 |>
  select(SUR_MON_YR, VDS_ID, PRE_MEM_ID, OLD_MEM_ID, REL, GENDER, AGE, WORK_TYPE,
         WORK_DAYS, AVG_WORK_HRS, INV_UNEMP_DAYS, OWN_FARM_D, OWN_FARM_H,
...
Pour ce deuxième cas, j'additionne les heures travaillées par mois des différents travaux employés par individu. 
...
Problème : je trouve que les heures de travail par mois sont trop hautes, les gens travaillent pas tant. Je pense qu'il ne fait pas additionner tous les doublons des différents travaux employés 
...
Idée: filtrer is.na(WORK_TYPE) et conserver pour les doublons la ligne avec le moins de n/a. 
