---
title: "Contrôler pour la méchanisation : Analyse du travail féminin familial par caste"
format: html
editor: source
---
...
```{r, message=FALSE, warning=FALSE}
here::i_am("ICRISAT_database_analysis/Analyse_mecanisation_2014.qmd")
library(dplyr)
library(tidyr)
library(readxl)
library(ggplot2)
...
```{r}
outils_per_oper <- Cultivation_expand |>
  filter(
    SEASON == "KHARIF", CROP == "PADDY",
    !OPERATION %in% c("HAND WEEDING", "SUPERVISION", "MARKETING", "OTHERS")) |>
  select(SUR_MON_YR, VDS_ID, PLOT_CODE, AREA_CROP, OPERATION, DT_OPER, LAB_TYPE, NAME_MAT) |>
  distinct()
```
...
outils_per_oper |>
  ...
  summarise(
    total_oper = n(),
    n_TR = sum(NAME_MAT == "TR", na.rm = TRUE),
    prct_TR = round(100*n_TR / total_oper, 0),
    n_TH = sum(NAME_MAT == "TH", na.rm = TRUE),
    prct_TH = round(100*n_TH / total_oper, 0),
    n_SP = sum(NAME_MAT == "SP", na.rm = TRUE),
    prct_SP = round(100*n_SP / total_oper, 0),
    n_ET = sum(NAME_MAT == "ET", na.rm = TRUE),
    prct_ET = round(100*n_ET / total_oper, 0),
    .groups = "drop"
  ) |>
  arrange(desc(total_oper))
```
...
mecha_fertilizer <- mecha_by_area |>
  filter(OPERATION == "FERTILIZER APPL.") |>
  group_by(bin_size) |>
  summarise(
    total = n(),
    mechanized = sum(mech_used == "TRUE"),
