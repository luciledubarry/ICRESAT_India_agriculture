...
```{r}
#| message: false
here::i_am("ICRISAT_database_analysis/scripts_2014/CultData2_cleaning_2014.qmd")
library(dplyr)
library(tidyr)
library(knitr)
library(readxl)
...
```{r}
# Diviser CROP_AREA pour chaque culture, selon les variables PRCT_AREA
Cultivation_cult_distinctes <- Cultivation_2014 |>
  mutate(
    AREA_CROP_1 = CROP_AREA * PRCT_AREA_1 / 100,
    AREA_CROP_2 = if_else(!is.na(PRCT_AREA_2), CROP_AREA * PRCT_AREA_2 / 100, NA_real_),
    AREA_CROP_3 = if_else(!is.na(PRCT_AREA_3), CROP_AREA * PRCT_AREA_3 / 100, NA_real_),
    AREA_CROP_4 = if_else(!is.na(PRCT_AREA_4), CROP_AREA * PRCT_AREA_4 / 100, NA_real_)
  )
...
```{r}
Cultivation_cult_distinctes <- Cultivation_cult_distinctes |>
  select(-WORK_HR, -WAGE_PER_HR, -WAGE, -IRRI_AREA, -CROP_AREA, -OPER_ID)
...
Cultivation_cult_distinctes <- Cultivation_cult_distinctes |>
  pivot_longer(
    cols = matches("^(CROP|CROP_ID|PRCT_AREA|OP_MAIN_PROD_QTY|OP_MAIN_PROD_UNIT|VAR_NAME|VAR_TYPE|AREA_CROP|IRRI_CROP|WORK_HR)_[1-4]$"),
    names_to = c(".value", "NB"),
    names_pattern = "(.*)_(\\d)"
  )
```
...
Avant de recatégoriser les opérations en une dizaine d'opérations, je crée la variable Herbicide qui indique si l'opération a nécessité de l'herbicide/weedicide.

```{r}
# Variable HERBICIDE: indique TRUE si l'une des deux conditions est remplie. 
Cultivation_cult_distinctes <- Cultivation_cult_distinctes |>
  mutate(
    HERBICIDE = OPERATION == "HERBICIDE" | 
    replace_na(str_detect(TYPE_MAT, "Weed|Herb"), FALSE)
    # considère FALSE si TYPE_MAT = n/a
  )
```
...
multi_crop <- Cultivation_cult_distinctes |>
  distinct(VDS_ID, SUB_PLOT_CODE, PLOT_CODE, SEASON, CROP) |>
  group_by(VDS_ID, SUB_PLOT_CODE, PLOT_CODE, SEASON) |>
  summarise(MULTI_CROPING = n_distinct(CROP, na.rm = TRUE) > 1, .groups = "drop")

Cultivation_cult_distinctes <- Cultivation_cult_distinctes |>
  left_join(multi_crop, by = c("VDS_ID", "SUB_PLOT_CODE", "PLOT_CODE", "SEASON"))
```
...
cat("Sur un total de", nrow(nb_champ), "lignes soit", round(100 * nrow(nb_multi_croping) / nrow(nb_champ), 2), "%\n")
```
...
```{r}
first_crop_village <- Cultivation_cult_distinctes |>
...
```{r}
households_with_paddy <- Cultivation_cult_distinctes |>
...
```{r}
# Superficie totale cultivée par ménage et par saison
hh_area <- Cultivation_cult_distinctes |>
  distinct(VDS_ID, PLOT_CODE, SUB_PLOT_CODE, AREA_CROP, SEASON) |>
  group_by(VDS_ID, SEASON) |>
  summarise(AREA_HH = sum(AREA_CROP, na.rm = TRUE), .groups = "drop")

# Superficie et part de la terre dédiée au riz, par saison
paddy_area <- Cultivation_cult_distinctes |>
  distinct(VDS_ID, PLOT_CODE, SUB_PLOT_CODE, AREA_CROP, CROP, SEASON) |>
  filter(CROP == "PADDY") |>
  group_by(VDS_ID, SEASON) |>
  summarise(paddy_area = sum(AREA_CROP, na.rm = TRUE), .groups = "drop") |>
  left_join(hh_area, by = c("VDS_ID", "SEASON")) |>
  mutate(paddy_prct = if_else(AREA_HH > 0, paddy_area/AREA_HH, NA_real_),
         paddy_prct = round(100*paddy_prct, 0))
```

```{r}
# Joindre les variables AREA_HH et prct_paddy à la base principale
Cultivation_cult_distinctes <- Cultivation_cult_distinctes |>
  left_join(hh_area, by = c("VDS_ID", "SEASON")) |>
  left_join(paddy_area |> select(VDS_ID, SEASON, paddy_prct), by = c("VDS_ID", "SEASON")) |>
  relocate(AREA_HH, .after = VDS_ID)
```
...
plots_by_household <- Cultivation_cult_distinctes |>
  distinct(
    VDS_ID, PLOT_CODE, SUB_PLOT_CODE, SEASON, CROP, CROP_ID, AREA_HH,
    PLOT_AREA, AREA_CROP, IRRI_CROP, VAR_TYPE, VAR_NAME,
    MULTI_CROPING, REGION, STATE, VILLAGE, VILLAGE_CROP, with_paddy,
    paddy_prct, SOIL_TYPE, SOIL_TYPE_OT, SOIL_DEPTH, SOIL_FERT,
    SLOPE, SOIL_DEGR
           )
...
base_to_join <- Cultivation_cult_distinctes |>
  select(
    ...
    MONTH, GENDER, EMPLOYMENT, HUMAN_LABOR, MACHINERY, DT_OPER,
    SUR_MON_YR, FERTILIZER, HERBICIDE, LAB_TYPE, WORK_HR, 
    OP_MAIN_PROD_UNIT, OP_MAIN_PROD_QTY, NAME_MAT, TYPE_MAT
  )
...
```{r, message=FALSE, warning=FALSE}
EAST_Household_info <- read_excel(here::here("raw_data", "EAST_Household_2014.xlsx"))
SAT_Household_info <- read_excel(here::here("raw_data", "SAT_Household_2014.xlsx"))
```
...
Cultivation_expand <- Cultivation_expand |>
  select(
    SUR_MON_YR, MONTH, SEASON, VDS_ID, CASTE, CASTE_GROUP, PLOT_CODE, SUB_PLOT_CODE, AREA_HH, PLOT_AREA, AREA_CROP, IRRI_CROP, CROP, IRRIGATION, DT_OPER, OPERATION, LAB_TYPE, WORK_HR, GENDER, EMPLOYMENT, NAME_MAT, TYPE_MAT, OP_MAIN_PROD_UNIT, OP_MAIN_PROD_QTY, MACHINERY, HERBICIDE, FERTILIZER, VAR_TYPE, VAR_NAME, CROP_ID, HUMAN_LABOR, with_paddy, paddy_prct, MULTI_CROPING, REGION, STATE, VILLAGE, VILLAGE_CROP, everything()  # place les colonnes restantes à la fin
  )
```
...
Je repère les champs pour lesquels les variables VDS_ID, SUB_PLOT_CODE et SEASON ne sont pas suffisantes pour les identifier de façon unique et qui doivent être différenciés avec CROP. 

```{r}
# Tableau des champs nécessitant CROP pour les identifier
dup_crop <- Cultivation_expand |>
  select(VDS_ID, SUB_PLOT_CODE, SEASON, CROP, AREA_CROP) |>
  distinct() |>
  group_by(VDS_ID, SUB_PLOT_CODE, SEASON, AREA_CROP) |>
  filter(n() > 1) |>
...
Pour cela, je pivote en large la base selon la saison (pour les variables AREA_CROP et CROP). Le but est d'indiquer les potentielles différences de superficie d'un chap entre les saisons et de repérer les champs manquants (donc en jachère). 
...
Les autres doublons présents dans Cultivation_expand sont dûs à des différences dans VAR_NAME ET VAR_TYPE, donc correspondent à des champs distincts. 
